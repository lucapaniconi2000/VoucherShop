# Docs Index

These docs explain how VoucherShop is structured, how to run it locally, how the API works, and how to deploy it (provider-agnostic).

## Contents
- [01 — Architecture](01-architecture.md)
- [02 — API Overview](02-api.md)
- [03 — Database & Migrations](03-database-and-migrations.md)
- [04 — Auth & Security](04-auth-and-security.md)
- [05 — Deployment (provider-agnostic)](05-deployment.md)
- [06 — Troubleshooting](06-troubleshooting.md)
